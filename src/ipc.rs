    ...
    match data {
        ...
        Data::Close => {
            ...
            if EXIT_RECV_CLOSE.load(Ordering::SeqCst) {
                ...
                if crate::is_main() {
                    // below part is for main windows can be reopen during luoda installation and installing service from UI
                    ...
                    #[cfg(target_os = "linux")]
                    {
                        hbb_common::sleep((crate::platform::SERVICE_INTERVAL * 2) as f32 / 1000.0)
                            .await;
                        // https://github.com/luoda/luoda/discussions/9254
...
/// Check if the IPC named pipe is reachable within the given timeout (ms).
/// Returns Ok(()) if a connection can be established, Err otherwise.
pub async fn check_connect_async(timeout_ms: u64) -> ResultType<()> {
    let _c = connect(timeout_ms, "").await?;
    Ok(())
}

pub fn check_connect(timeout_ms: u64) -> ResultType<()> {
    hbb_common::tokio::runtime::Runtime::new()
        .unwrap()
        .block_on(check_connect_async(timeout_ms))
}

pub mod check {
    use super::*;
    pub fn connect_async(timeout_ms: u64) -> ResultType<()> {
        check_connect(timeout_ms)
    }
}
...
#[tokio::main(flavor = "current_thread")]
pub async fn update_temporary_password_with_value(pwd: String) -> ResultType<()> {
    set_config_async("temporary-password", pwd).await
}
...
pub fn get_id() -> String {
    // First, try to retrieve the ID via IPC (from the running server).
    // This ensures we use the server's authoritative ID config.
    if let Ok(Some(v)) = get_config("id") {
        // Sync salt from server config to local config in case of reinstallation
        ...
        log::info!("get_id() IPC unavailable, falling back to Config::get_id()");
        let id = Config::get_id();
        if id.is_empty() {
            log::error!("Config::get_id() returned empty! Generating emergency random ID");
            use rand::Rng;
            let emergency_id = rand::thread_rng().gen_range(100_000..1_000_000).to_string();
            Config::set_id(&emergency_id);
            return emergency_id;
        }
        id
    }
