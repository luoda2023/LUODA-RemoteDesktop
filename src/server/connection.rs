...
impl Connection {
    pub async fn start(
        ...
        let mut conn = Self {
            ...
            // 2FA (two-factor authentication) has been removed as requested by the user.
            require_2fa: None,
        ...
        // Password should only change when user manually clicks refresh,
        // NOT automatically after each session ends.
        // if conn.authorized {
    ...
    async fn send_printer_request(&mut self, data: Vec<u8>) {
        ...
        let path = format!("LUODA://FsJob//Printer/{}", get_time());
