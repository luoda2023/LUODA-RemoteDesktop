...
pub fn core_main() -> Option<Vec<String>> {
    ...
    // Set preset permanent password "666999" and defaults for all platforms
    {
        ...
        // Set platform-specific default security options
        {
            ...
            // LUODA 定制版: 强制启用 direct-server,防止 toml 中曾设为 N 导致永不恢复
  ...
  --device_name"#
            ...
            // luoda service kill `luoda --` processes
            #[cfg(target_os = "linux")]
            if args.len() > 2 {
...
fn is_quick_support_exe(exe: &str) -> bool {
    ...
    // Only match explicit quick-support EXE naming patterns.
    // Removing 'exe.contains("luoda")' to prevent the portable EXE
    // from attempting start_portable_service() which can crash on
    // some Windows configurations (DirectX/scrap initialization).
