...
const IDD_IMPL_LUODA: &str = "luoda_idd";
...
/// every time the RDP session drops on a headless VPS.
///
...
pub fn get_platform_additions() -> serde_json::Map<String, serde_json::Value> {
    ...
    match IDD_IMPL {
        IDD_IMPL_LUODA => {
            let virtual_displays = luoda_idd::get_virtual_displays();
            if !virtual_displays.is_empty() {
                map.insert(
                    "luoda_virtual_displays".into(),
...
pub fn plug_in_monitor(idx: u32, modes: Vec<virtual_display::MonitorMode>) -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_LUODA => luoda_idd::plug_in_index_modes(idx, modes),
    ...
    match IDD_IMPL {
        IDD_IMPL_LUODA => {
            let indices = if index == IDD_PLUG_OUT_ALL_INDEX {
                luoda_idd::get_virtual_displays()
            } else {
                vec![index as _]
            };
            luoda_idd::plug_out_peer_request(&indices)
        }
...
pub fn plug_in_peer_request(modes: Vec<Vec<virtual_display::MonitorMode>>) -> ResultType<Vec<u32>> {
    match IDD_IMPL {
        IDD_IMPL_LUODA => luoda_idd::plug_in_peer_request(modes),
        IDD_IMPL_AMYUNI => {
...
pub fn plug_out_monitor_indices(
    ...
    match IDD_IMPL {
        IDD_IMPL_LUODA => luoda_idd::plug_out_peer_request(indices),
        IDD_IMPL_AMYUNI => {
...
pub fn reset_all() -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_LUODA => luoda_idd::reset_all(),
...
pub mod amyuni_idd {
    ...
    //             if there're no virtual displays managed by LUODA.
    pub fn plug_out_monitor(index: i32, force_all: bool, force_one: bool) -> ResultType<()> {
            ...
            Err(e) => {
                ...
                if let Ok(exe) = std::env::current_exe() {
                    ...
                    match crate::platform::run_uac(&exe, "--install-amyuni-idd") {
                        ...
                        Err(err) => {
                            log::warn!("failed to launch elevated amyuni installer: {}", err)
                        }
                    }
                }
            }
