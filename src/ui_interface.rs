...
pub fn set_option(key: String, value: String) {
    if &key == "stop-service" {
        ...
        {
            if value == "Y" {
                // Stop: don't uninstall service - just tell mediator to stop accepting connections
                // Full uninstall/reinstall causes 1-2s delay and window flash
                crate::rendezvous_mediator::RendezvousMediator::restart();
            } else if value.is_empty() || value == "false" {
                // Start: if already installed, fast path - just restart mediator
                // No need to reinstall service (that kills processes and flashes window)
                if crate::platform::is_installed() {
                    crate::rendezvous_mediator::RendezvousMediator::restart();
                } else {
                    // Portable mode (not installed as service):
                    // Never call install_service() here - that runs taskkill and exits the process!
                    // Just update the config and restart mediator to re-enable connections.
                    let mut options = OPTIONS.lock().unwrap();
                    options.remove("stop-service");
                    let o = options.clone();
                    drop(options);
                    let _ = ipc::set_options(o);
                }
            }
        }
...
pub fn temporary_password() -> String {
    ...
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    {
        let pwd = TEMPORARY_PASSWD.lock().unwrap().clone();
        if pwd.is_empty() {
            // First access: sync from password_security so the UI shows the same
            // password the server/service is using (not the empty placeholder).
            let real = password_security::temporary_password();
            *TEMPORARY_PASSWD.lock().unwrap() = real.clone();
            real
        } else {
            pwd
        }
...
pub fn update_temporary_password() {
    ...
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    {
        // Generate the password locally first, then send the EXACT value to the service
        // via IPC. This ensures the UI process and the service process use the same password.
        password_security::update_temporary_password();
        let pwd = password_security::temporary_password();
        *TEMPORARY_PASSWD.lock().unwrap() = pwd.clone();
        allow_err!(ipc::update_temporary_password_with_value(pwd));
    }
}
...
async fn check_connect_status_(reconnect: bool, rx: mpsc::UnboundedReceiver<ipc::Data>) {
    ...
    loop {
        if let Ok(mut c) = ipc::connect(1000, "").await {
            let mut timer = crate::luoda_interval(time::interval(time::Duration::from_secs(1)));
            loop {
                tokio::select! {
                    res = c.next() => {
                        match res {
