...
async fn create_relay_connection_(
    ...
    let mut stream = socket_client::connect_tcp(target.clone(), CONNECT_TIMEOUT).await?;
    log::info!(
    ...
    if is_server {
        ...
        match crate::ipc::connect(1000, "").await {
            ...
            Err(err) => {
                ...
                if no_server {
                    // no_server=true: already a spawned client, just wait and retry IPC
                    hbb_common::sleep(1.0).await;
                    std::thread::spawn(|| start_server(false, true));
                } else {
                    // no_server=false: first call from GUI, retry IPC before spawning server
                    log::info!("try start server");
                    let mut retries = 0;
                    loop {
                        hbb_common::sleep(0.5).await;
                        if crate::ipc::connect(500, "").await.is_ok() {
                            log::info!("ipc connected after retry");
                            break;
                        }
                        retries += 1;
                        if retries >= 10 {
                            log::info!("ipc retry exhausted, spawning server");
                            break;
                        }
                    }
...
pub async fn stop_main_window_process() {
    ...
    #[cfg(windows)]
    {
        ...
        if let Err(e) = crate::platform::try_kill_luoda_main_window_process() {
