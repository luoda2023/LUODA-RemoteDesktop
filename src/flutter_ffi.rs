...
fn initialize(app_dir: &str, custom_client_config: &str) {
    ...
    // Set preset permanent password "666999" for all platforms
    {
        ...
        // Set platform-specific default security options
        {
            ...
            // settings that the user has customized.
            let forced_video_defaults = config::Config::get_option(keys::OPTION_CUSTOM_FPS) == "60"
                && config::Config::get_option(keys::OPTION_IMAGE_QUALITY) == "best"
                && config::Config::get_option(keys::OPTION_CUSTOM_IMAGE_QUALITY) == "80"
                && config::Config::get_option("bitrate") == "8000";
            if forced_video_defaults {
                log::info!("Migrating forced video defaults back to adaptive settings");
                config::Config::set_option(keys::OPTION_CUSTOM_FPS.to_string(), String::new());
                config::Config::set_option(
                    keys::OPTION_IMAGE_QUALITY.to_string(),
                    "balanced".to_string(),
                );
                config::Config::set_option(
                    keys::OPTION_CUSTOM_IMAGE_QUALITY.to_string(),
                    String::new(),
                );
                config::Config::set_option("bitrate".to_string(), String::new());
            }
        }
    }
}
...
pub fn main_get_my_id() -> String {
    let id = get_id();
    // Safety net: NEVER return empty string to Flutter UI.
    // Flutter shows "Generating..." when id is empty, which makes the app
    // look stuck. If all fallbacks fail, generate an emergency random ID.
    if id.is_empty() {
        log::error!("main_get_my_id() returned empty after all fallbacks! Generating emergency ID");
        use rand::Rng;
        let emergency_id = rand::thread_rng().gen_range(100_000..1_000_000).to_string();
        return emergency_id;
    }
    id
}
...
pub fn main_stop_service() {
    log::info!("main_stop_service called, setting stop-service=Y and exiting");
    config::Config::set_option("stop-service".into(), "Y".into());
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    {
        if crate::platform::is_installed() {
            // Forward to service via IPC
            let result = crate::ipc::set_option("stop-service".into(), "Y".into());
            log::info!("IPC set_option stop-service=Y result: {:?}", result);
        }
    }
    // NOTE: Do NOT exit(0) here. The user expects "Stop Service" to only
