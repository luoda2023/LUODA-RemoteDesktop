    ...
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    {
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            INIT.call_once(|| {
                ...
                // https://github.com/luoda/luoda/blob/85db6779828349b23ca3eba91cc7cd36c5337797/src/common.rs#L822
...
pub fn init_log(_is_async: bool, _name: &str) -> Option<flexi_logger::LoggerHandle> {
    ...
    INIT.call_once(|| {
        ...
        {
            use env_logger::*;
            let _ = try_init_from_env(Env::default().filter_or(DEFAULT_FILTER_ENV, "info,reqwest=warn,rustls=warn,webrtc-sctp=warn,webrtc=warn"));
        }
