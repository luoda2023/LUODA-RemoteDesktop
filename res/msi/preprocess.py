...
def make_parser():
    ...
    parser.add_argument(
        "--app-name", type=str, default="LDesk", help="The app name."
    )
    parser.add_argument(
...
def init_global_vars(dist_dir, app_name, args):
    ...
    if g_version == "":
        raw = read_process_output("--version")
        # Flutter/Sciter builds may output log lines before the version string.
        # Search for 'vX.Y.Z' or 'X.Y.Z' pattern anywhere in the output.
        version_pattern = re.compile(r"v?(\d+\.\d+\.\d+)")
        m = version_pattern.search(raw)
        if m:
            g_version = m.group(1)
        else:
            print(f"Error: version pattern not found in output: {raw[:200]}")
            return False
    version_validate = re.compile(r"\d+\.\d+\.\d+")
    if not version_validate.match(g_version):
    ...
    raw_date = read_process_output("--build-date")
    build_date_pattern = re.compile(r"\d{4}-\d{2}-\d{2} \d{2}:\d{2}")
    m = build_date_pattern.search(raw_date)
    if m:
        g_build_date = m.group(0)
    else:
        print(f"Warning: build date not found in output: {raw_date[:200]}, using default")
        # Keep default date set at top of script
...
def update_license_file(app_name):
    ...
    license_content = license_content.replace("website luoda.com and other ", "")
